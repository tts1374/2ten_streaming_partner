...
YouTube Live Chatを実入力にする場合は、`config/local.toml`の`[youtube_chat]`に`live_chat_id`または`video_id`を設定し、`api_key_env`で指定した環境変数、既定では`YOUTUBE_API_KEY`にYouTube Data APIキーを入れてから`--use-youtube-chat`を付ける。`video_id`を指定した場合は、起動時にYouTube Data APIの`videos?part=liveStreamingDetails`から`activeLiveChatId`を解決する。CLIでは`--youtube-video-id`で一時的に動画IDを上書きできる。初期実装はYouTube Live Streaming APIの`liveChat/messages`を標準ライブラリでポーリングし、`nextPageToken`、`pollingIntervalMillis`、メッセージIDの重複排除を使って新規コメントだけを`InputEvent(source="youtube_chat")`へ正規化する。1回のpollで複数コメントが返った場合は、長すぎるコメント、記号比率が高いコメント、同一poll内および短時間内の同文重複、単一文字の反復が多いコメントを事前フィルタし、`youtube_chat.max_selected_per_poll`件まで軽量スコアで選定する。ゼロ幅文字などの制御文字は同文判定時に除去する。選定順位、スコア、除外件数はmetadataに残す。`--use-llm-chat-selection`を付けた場合は、軽量選定後の候補を`qwen3.5:4b`の`selection`用途に渡し、つてんに振る価値が高い1件だけを採用する。LLM選定のJSONが壊れている、範囲外indexを返す、または採用なしを返す場合はfail closedとしてそのpollの候補を拾わない。採用イベントのmetadataには選定モデル、理由、信頼度、候補数を残し、モデル呼び出し自体は`llm_calls`へ記録する。`skip_initial_history = true`が既定なので、起動直後に返る過去コメントには反応せず、次ページ以降の新規コメントを拾う。
...
複数コメントからローカルLLMで1件へ絞る場合は、Ollama経路と併用して以下のように起動する。

```bash
uv run aituber-partner --use-youtube-chat --use-ollama --use-llm-chat-selection --fast-output-safety --serve-overlay --config config/local.toml
```
...
STT入力は、録音、無音検出、Whisper-family推論を直接Orchestratorへ結合せず、まず`TranscriptProvider`が完了済みの`TranscriptionResult`を返し、`STTInputSource`が`InputEvent(source="voice")`へ正規化する。これにより、faster-whisper、別プロセスSTT、手動テスト用transcript providerを同じ入力契約へ差し替えられる。空文字や設定した信頼度未満のsegmentは入力イベント化しない。採用したvoice eventのmetadataには、STTモデル、言語、confidence、segment秒数などを残す。
...
選定は、安全性、話題性、文脈適合度、返答しやすさをスコア化する。YouTube Chatでは入力段の軽量スコアで候補数を絞り、`--use-llm-chat-selection`指定時だけ後段のLLM選定を実行する。LLM選定はJSONで`selected_index`、`reason`、`confidence`を返させ、パース失敗時は安全側に倒して不採用にする。
...
drop_symbol_heavy_messages = true
symbol_heavy_threshold = 0.6
drop_duplicate_text_per_poll = true
recent_duplicate_text_window_seconds = 30.0
drop_repetitive_messages = true
repetitive_text_threshold = 0.7
repetitive_text_min_length = 8
...
- 録音/VAD/Whisper推論は`TranscriptProvider`として実装し、`STTInputSource`で音声入力イベントをチャット入力と同じ経路に流す。
- 低信頼度や空文字segmentは破棄し、採用segmentにはSTTモデルと言語、confidence、durationをmetadataとして残す。
