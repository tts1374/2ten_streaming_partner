...
YouTube Live Chatを実入力にする場合は、`config/local.toml`の`[youtube_chat]`に`live_chat_id`または`video_id`を設定し、`api_key_env`で指定した環境変数、既定では`YOUTUBE_API_KEY`にYouTube Data APIキーを入れてから`--use-youtube-chat`を付ける。`video_id`を指定した場合は、起動時にYouTube Data APIの`videos?part=liveStreamingDetails`から`activeLiveChatId`を解決する。CLIでは`--youtube-video-id`で一時的に動画IDを上書きできる。初期実装はYouTube Live Streaming APIの`liveChat/messages`を標準ライブラリでポーリングし、`nextPageToken`、`pollingIntervalMillis`、メッセージIDの重複排除を使って新規コメントだけを`InputEvent(source="youtube_chat")`へ正規化する。1回のpollで複数コメントが返った場合は、長すぎるコメント、記号比率が高いコメント、同一poll内の同文重複を事前フィルタし、`youtube_chat.max_selected_per_poll`件まで軽量スコアで選定する。選定順位、スコア、除外件数はmetadataに残す。`skip_initial_history = true`が既定なので、起動直後に返る過去コメントには反応せず、次ページ以降の新規コメントを拾う。
